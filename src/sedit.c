...
extern const char *shop_bits[];			/*. shop.h	.*/
extern const char *item_types[];		/*. constants.c	.*/
...
{ int i;
  ...
  if (arg && *arg) {
    ...
    if (OLC_MODE(d) == SEDIT_HELP) return;
  } 
  ...
  case SEDIT_HELP:
    ...
    switch (OLC_MODE(d)) {
      case SEDIT_MAIN_MENU:
        ...
        break;
      default:
        log("SYSERR: Help topic with undefined return in sedit (mode = %d)", OLC_MODE(d));
        break;
    }
    return;
/*-------------------------------------------------------------------*/
