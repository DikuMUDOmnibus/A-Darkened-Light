...
char *list_object(struct char_data * ch, struct obj_data * obj, int cnt, int index, int shop_nr);
...
int buy_price(struct char_data * ch, struct obj_data * obj, int shop_nr)
{
  float profit = (SHOP_BUYPROFIT(shop_nr) > 1) ? 
    (1 + (SHOP_BUYPROFIT(shop_nr) - 1) * 9 / (2*GET_CHA(ch)))
	...
	int index, int shop_nr)
{
  ...
  if (shop_producing(obj, shop_nr))
    ...
    sprintf(buf2, "%5d       ", cnt);
  sprintf(buf, " &c%2d&w)  &g%s&w", index, buf2);
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  one_argument(arg, name);
  strcpy(buf, "&W ##   Available   Item                                               Cost\n\r");
  strcat(buf, "&w-------------------------------------------------------------------------\n\r");
  if (keeper->carrying)
