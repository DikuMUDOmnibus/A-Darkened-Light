...
//  if ((GET_COND(ch, FULL) > 1000) && (GET_COND(ch, THIRST) > 1000)) {
  if ((GET_COND(ch, FULL) > 24) && (GET_COND(ch, THIRST) > 24)) {
    send_to_char("Your stomach can't contain anymore!\r\n", ch);
    return;
  }
//   if (GET_COND(ch, THIRST) > 0 ) {
//      send_to_char("You quench your thirst for blood and calm your 
//madness.\r\n", ch);
//      return;}
  ...
  if (subcmd == SCMD_DRINK) {
    ...
    sprintf(buf, "You drink the %s from $p.\r\n", drinks[GET_OBJ_VAL(temp, 
2)]);
  ...
  gain_condition(ch, THIRST,
	(int) ((int) drink_aff[GET_OBJ_VAL(temp, 2)][THIRST] * amount) / 20);

  if (GET_COND(ch, DRUNK) > 10)
    send_to_char("You feel drunk.\r\n", ch);

//  if (GET_COND(ch, THIRST) > 1000)
  if (GET_COND(ch, THIRST) > 24)
    send_to_char("You blood thirst has been sated.\r\n", ch);

//  if (GET_COND(ch, FULL) > 1000)
...
//  if (GET_COND(ch, FULL) > 1000) {/* Stomach full */
  if (GET_COND(ch, FULL) > 24) {/* Stomach full */
...
ACMD(do_wield)
{
  ...
  if (PLR_FLAGGED(ch, PLR_CLAWS)) {
   send_to_char("You cannot wield anything while you have claws!", ch);
   return;
}
...
ACMD(do_grab)
{
  ...
  if (PLR2_FLAGGED(ch, PLR2_NOLARM)) { 
   send_to_char("You cannot hold anything with a missing left arm.\r\n",ch);
   return;
   }

  if (PLR_FLAGGED(ch, PLR_CLAWS)) {
   send_to_char("You cannot hold anything while you have claws!", ch);
   return;
}

  if (!*arg)
    send_to_char("Hold what?\r\n", ch);
 // else if (PLR_FLAGGED(ch, PLR2_NOLARM))
  //   send_to_char("You cannot hold anything with a missing left 
//arm.\r\n",ch);
  else if (!(obj = get_obj_in_list_vis(ch, arg, ch->carrying))) {
    sprintf(buf, "You don't seem to have %s %s.\r\n", AN(arg), arg);
    send_to_char(buf, ch);
