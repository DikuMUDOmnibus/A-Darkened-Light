   ...
   switch (command) { 
    case PARSE_HELP: 
      sprintf(buf, 
	      ...
	      "/h [topic] -  list text editor commands or display help\r\n"
	      "              dicussing specified topic\r\n"
	      ...
	      "/s         -  saves text\r\n"
	      "/t         -  toggles color code interpreting\r\n");
      SEND_TO_Q(buf, d);
      break;
    case PARSE_FORMAT: 
	 ...
	 *s = temp;
      ...
      sprintf(buf, "%s\r\n%d line%sshown.\r\n", buf, total_len,
	      ((total_len != 1)?"s ":" "));
       */
      show_color_codes = PRF_FLAGGED(d->character, PRF_EDIT_BW) ? TRUE : FALSE;
  ...
  delete_doubledollar(str);
   ...
   if ((action = (*str == '/'))) {
	 ...
	 break;
	   ...
	   do_help(d->character, argptr, 0, 0);
	 }
	 break;
       case 'l':
	 if (*d->str)
	 ...
	 break;
       case 't':
         if (TOGGLE_BIT(PRF_FLAGS(d->character), PRF_EDIT_BW) & PRF_EDIT_BW)
           send_to_char("Color codes &gON&w\r\n", d->character);
         else
           send_to_char("Color codes &rOFF&w\r\n", d->character);
         break;
       default:
	 SEND_TO_Q("Invalid option.\r\n", d);
	 break;
  ...
  if (terminator) {
	...
	*d->str = NULL;
	...
	if ((terminator == 1) && *d->str) {
      store_mail(d->mail_to, GET_IDNUM(d->character), d->obj_mail, *d->str);
	   SEND_TO_Q("Message sent!\r\n", d);
	}
	else SEND_TO_Q("Mail aborted.\r\n", d);
      d->mail_to = 0;
      if (d->obj_mail) {
//        obj_to_char(d->obj_mail, d->character);
        d->obj_mail = NULL;
      };
      free(*d->str);
      free(d->str);
	...
	if (terminator == 1) {
     ...
     if (d->character && !IS_NPC(d->character) && STATE(d) != CON_CLAN_EDIT
       && STATE(d) != CON_OEDIT && STATE(d) != CON_ZEDIT && STATE(d) != CON_REDIT
...
ACMD(do_skillset)
{
  ...
  if (!*name) {			/* no arguments. print an informative text */
    ...
    for (i = 0; *spells[i] != '\n'; i++) {
      if (*spells[i] == '!')
	continue;
      sprintf(help + strlen(help), "%-26s", spells[i]);
      if (i % 3 == 2) {
	strcat(help, "\r\n");
...
char *next_page(char *str)
{
  int col = 1, line = 1, spec_code = FALSE, color_code = FALSE;
