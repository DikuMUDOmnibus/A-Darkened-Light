...
/* IMPORTANT NOTE: This is now obsolete and never called in spec_assign.c (YIL)*/
...
void fry_victim(struct char_data * ch)
{
  ...
  case 7:
    ...
    cast_spell(ch, tch, 0, 0, SPELL_FIREBALL);
    break;
  default:
    if (!number(0, 1))
      cast_spell(ch, ch, 0, 0, SPELL_HEAL);
    break;
  }
