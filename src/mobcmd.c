...
char *mprog_type_to_name(int type)
{
    ...
    case RESET_PROG:            return "reset_prog";
    case TIME_PROG:             return "time_prog";
    case NO_PROG:		return "no mob program";
    default:                    return "error report to Implementor!!";
    }
}

/* Return argument string for help */

char * mprog_get_argument_str(int type)
{
  switch (type) {
    case IN_FILE_PROG:          return "Program file";
    case ACT_PROG:              return "Keywords ([p]hrase)";
    case SPEECH_PROG:           return "Keywords ([p]hrase)";
    case RAND_PROG:             return "Percentage";
    case FIGHT_PROG:            return "Percentage";
    case HITPRCNT_PROG:         return "Percentage";
    case DEATH_PROG:            return "Percentage";
    case ENTRY_PROG:            return "Percentage";
    case GREET_PROG:            return "Percentage";
    case ALL_GREET_PROG:        return "Percentage";
    case GIVE_PROG:             return "Object name";
    case BRIBE_PROG:            return "Money amount";
    case LEAVE_PROG:            return "Percentage";
    case RESET_PROG:            return "Percentage";
    case TIME_PROG:             return "hour";
    case NO_PROG:		return "no mob program";
    default:                    return "-";
  }
...
ACMD(do_mpjunk)
{
    ...
    if (arg[0] == '\0') {
        bug("Mpjunk - No argument: vnum %d.", mob_index[ch->nr].vnum);
	return;
    }
...
ACMD(do_mpechoaround)
{
  ...
  char *p;
    ...
    if (arg[0] == '\0')
    {
       bug("Mpechoaround - No argument:  vnum %d.", mob_index[ch->nr].vnum);
       return;
    }
...
ACMD(do_mpmload)
{
    ...
    {
	bug("Mpmload - Bad vnum as arg: vnum %d.", mob_index[ch->nr].vnum);
	return;
    }

    if ((pMobIndex = get_mob_index(atoi(arg))) == NULL)
    {
	bug("Mpmload - Bad mob vnum: vnum %d.", mob_index[ch->nr].vnum);
	return;
    }
...
//	bug(arg1, 0);
    ...
    {
	bug("Mpoload - Bad vnum arg: vnum %d.", mob_index[ch->nr].vnum);
	return;
    }
...
ACMD(do_mppurge)
{
	...
	for (obj = world[ch->in_room].contents; obj != NULL; obj = obj_next)
	{
 	...
 	if ((obj = get_obj_vis(ch, arg))) {
 	    ...
 	    bug("Mppurge - Bad argument: vnum %d.",mob_index[ch->nr].vnum);
 	}
 	return;
     }
     ...
     if (!IS_NPC(victim))
     {
 	bug("Mppurge - Purging a PC: vnum %d.", mob_index[ch->nr].vnum);
 	return;
     }
...
ACMD(do_mpgoto)
{
	...
	return;
     ...
     if (arg[0] == '\0')
     {
 	bug("Mpgoto - No argument: vnum %d.", mob_index[ch->nr].vnum);
 	return;
     }
 
     if ((location = find_target_room(ch, arg)) < 0)
     {
 	bug("Mpgoto - No such location: vnum %d.", mob_index[ch->nr].vnum);
 	return;
     }
...
/*    struct char_data       *wch; */
    ...
    {
	bug("Mpat - Bad argument: vnum %d.", mob_index[ch->nr].vnum);
 	return;
    }

    if ((location = find_target_room(ch, arg)) < 0)
    {
 	bug("Mpat - No such location: vnum %d.", mob_index[ch->nr].vnum);
 	return;
    }
...
ACMD(do_mptransfer)
{
    ...
    if (arg1[0] == '\0')
    {
 	bug("Mptransfer - Bad syntax: vnum %d.", mob_index[ch->nr].vnum);
 	return;
    }
	...
	for (d = descriptor_list; d != NULL; d = d->next)
 	...
 	if (IS_SET(ROOM_FLAGS(location), ROOM_PRIVATE))
 	{
...
ACMD(do_mpforce)
{
    ...
    if (arg[0] == '\0' || argument[0] == '\0')
    {
	bug("Mpforce - Bad syntax: vnum %d.", mob_index[ch->nr].vnum);
 	return;
    }
...
ACMD(do_mpblock)
{
    ...
    if (arg[0] == '\0')
    {
	bug("Mpblock - Bad syntax: vnum %d.", mob_index[ch->nr].vnum);
 	return;
    }
	...
	for (i = descriptor_list; i ; i = i->next) {
           if(i->character != ch && !i->connected &&
             ...
             vch = i->character;
             if(GET_LEVEL(vch) <= GET_LEVEL(ch) && CAN_SEE(ch, vch)) {
 		SET_BIT(AFF2_FLAGS(vch), AFF2_BLOCK);
 	     }
 	   }
         }
     } else {
 	...
 	if ((victim = get_char_room_vis(ch, arg)) == NULL) {
 	    bug("Mpblock - No such victim: vnum %d.",
 	  	mob_index[ch->nr].vnum);
 	    return;
 	}
 
 	if (victim == ch) {
 	    bug("Mpblock - Blocking oneself: vnum %d.",
 	    	mob_index[ch->nr].vnum);
 	    return;
 	}
 
        if (GET_LEVEL(victim) <= GET_LEVEL(ch))
 	  SET_BIT(AFF2_FLAGS(victim), AFF2_BLOCK);
     }
