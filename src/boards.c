...
int Board_write_message(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  if (num_of_msgs[board_type] >= MAX_BOARD_MESSAGES) {
    send_to_char("&RThe board is full.\r\n&w", ch);
    return 1;
  }
  if ((NEW_MSG_INDEX(board_type).slot_num = find_slot()) == -1) {
    send_to_char("&RThe board is malfunctioning - sorry.\r\n&w", ch);
...
int Board_show_board(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  strcpy(buf,
	 "This is a bulletin board.  Usage: &WREAD/REMOVE/RESPOND&w <msg #>, &WWRITE&w <header>.\r\n"
	 "You will need to look at the board to save your message.\r\n");
  if (!num_of_msgs[board_type])
    strcat(buf, "&gThe board is empty.&w\r\n");
  else {
    sprintf(buf + strlen(buf), "There are &C%d&w messages on the board.\r\n",
	    num_of_msgs[board_type]);
    #if NEWEST_AT_TOP
	...
	send_to_char("&GSorry, the board isn't working.\r\n&w", ch);
	return 1;
      }
    }
  }
...
void Board_display_response(int board_type, int slot_num, char *output)
{
  ...
  sprintf(buf, "&M%-2d&w : &W%s&w\r\n", slot_num + 1, MSG_HEADING(board_type, slot_num));
  strcat(output, buf);
  #if NEWEST_AT_TOP
    for (k = num_of_msgs[board_type] - 1; k >= 0; k--)
  #else
    for (k = 0; k < num_of_msgs[board_type]; k++)
  #endif
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  if (MSG_SLOTNUM(board_type, ind) < 0 ||
    ...
    send_to_char("&RSorry, the board is not working.\r\n&w", ch);
    log("SYSERR: The board is fubar'd.");
    return 1;
  }
  if (!(MSG_HEADING(board_type, ind))) {
    send_to_char("&GThat message appears to be screwed up.\r\n&w", ch);
    return 1;
  }
  ...
  sprintf(buffer, "&WMessage &c%d&w : %s\r\n\r\n%s\r\n", msg,
...
int Board_remove_msg(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
...
#endif
  if (!MSG_HEADING(board_type, ind)) {
    send_to_char("&RThat message appears to be screwed up.\r\n&w", ch);
    return 1;
  }
  ...
  if (slot_num < 0 || slot_num >= INDEX_SIZE) {
    ...
    send_to_char("&RThat message is majorly screwed up.\r\n&w", ch);
    return 1;
  }
  for (i = 0; i < num_of_msgs[board_type]; i++)
...
int Board_respond_message(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  char *tmstr;
  int len, msg, ind;
  time_t ct;
