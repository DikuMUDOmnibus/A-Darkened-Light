...
{ 
  ...
  if(vzone_num > 526)
  { send_to_char("526 is the highest zone allowed.\r\n", ch);
    return;
  }
...
//  menubit(&ZON_FLAGS(OLC_ZONE(d)), zon_bits, buf, CHECK_MASK_PERM(d, zon_flags_mask));
  send_to_char(buf, d->character);
  sprintbit(ZON_FLAGS(OLC_ZONE(d)), zon_bits, buf1);
  sprintf(buf, "\r\n"
	"Current flags   : %s%s%s\r\n"
...
//    updatebit(&ZON_FLAGS(OLC_ZONE(d)), NUM_ZON_FLAGS, i, CHECK_MASK_PERM(d, zon_flags_mask));
    OLC_ZONE(d)->number = 1;
    zedit_disp_zon_flags(d);
    return;
/*-------------------------------------------------------------------*/
