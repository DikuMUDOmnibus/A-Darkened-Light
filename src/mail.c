...
Modified by Yilard of VisionMUD, ObjMail correction by Yilard
...
void report_items(struct char_data *ch, struct char_data *mailman,
  struct obj_data * obj, int *items, int *uitems);
...
void store_mail_object(char *str_buf, struct obj_data *obj)
{
   ...
   if (!obj)
     ...
     strcat(str_buf," 0");
     obj_to_string(obj, str_buf);
     strcat(str_buf,"\n");
     if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER) {
       for (obj1 = obj->contains; obj1; obj1 = obj1->next_content)
         store_mail_object(str_buf, obj1);
      strcat(str_buf," 1\n");
     }
   }
   return;
...
/*
 * void store_mail(long #1, long #2, struct obj_data * #3, char * #4)
 * #1 - id number of the person to mail to.
 * #2 - id number of the person the mail is from.
 * #3 - object attached to mail
...
void store_mail(long to, long from, struct obj_data *atch, char *message_pointer)
{
  ...
  char msg_buf[MAX_MAIL_SIZE];
  char *msg_txt = msg_buf;
  int bytes_written = 0;
  int total_length = strlen(message_pointer);
  struct char_data *i;


  strcpy(msg_buf, message_pointer);
  ...
  if (atch != NULL) {
    header.header_data.vnum = GET_OBJ_VNUM(atch);
    strcat(msg_txt, MAGIC_STRING);
    store_mail_object(msg_txt, atch);
    strcat(msg_txt, " 1\n");
    total_length = strlen(msg_txt);
    extract_obj(atch);
  } else
    header.header_data.vnum = -1;
//  log(msg_txt);
  ...
  for (i = character_list; i; i = i->next)
    if (GET_IDNUM(i) == to) {
      send_to_char("&CYou have new mail.&w\r\n", i);
      break;
    }
}				/* store mail */


struct obj_data *read_mail_object(char **str_buf)
{
  long prefix;
  struct obj_data *obj, *obj1;
  
  prefix = read_long(str_buf);
  if (prefix == 1) return NULL;
  if (prefix != 0) {
    log("SYSERR: read_mail_object - prefix is neither 0 nor 1");
    return NULL;
  }
  obj = obj_from_string(str_buf);
  if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER) {
    do {
      obj1 = read_mail_object(str_buf);
      if (obj1)
        obj_to_obj(obj1, obj);
    } while (obj1);
  }
   ...
   header (i.e. the text handed to the player) */
{
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  sprintf(buf, " &b* &g* &G* &Y* &WMidgaard Mail System &Y* &G* &g* &b*\r\n"
	  "&wDate: &W%s\r\n"
	  "  &wTo: &W%s\r\n"
	  "&wFrom: &W%s\r\n"
	  "&wAtch: &W%s&w\r\n\r\n", 
	    tmstr, to ? to : "Unknown",
	    from ? from : "Unknown",
	    real_object(obj_vnum) >= 0 ?
	    obj_proto[real_object(obj_vnum)].short_description : "None.");
  ...
  tmstr = strstr(message, MAGIC_STRING);
  if (tmstr == NULL)
    *obj = NULL;
  else {
    *tmstr = '\0';
    tmstr += strlen(MAGIC_STRING);
    *obj = read_mail_object(&tmstr);
  }
			  ...
			  int cmd, char *arg)
{
  ...
  if (*buf2 && (obj = get_obj_in_list_vis(ch, buf2, ch->carrying))) {
    items = 0;
    uitems = 0;
    report_items(ch, mailman, obj, &items, &uitems);
    if (items > MAX_MAIL_OBJECTS) {
      sprintf(buf, "$n tells you, 'Maximum of %d items may be mailed here in one package!'",
        MAX_MAIL_OBJECTS);
      act(buf, FALSE, mailman, 0, ch, TO_VICT);
      return;
    }
    if (uitems != 0) {
      sprintf(buf, "$n tells you, 'We cannot take any responsibility for handling such items! Better use courier.'");
      act(buf, FALSE, mailman, 0, ch, TO_VICT);
      return;
    }
    ...
    if (*buf2) {
      sprintf(buf, "$n tells you, 'You have no %s!'", buf2);
      act(buf, FALSE, mailman, 0, ch, TO_VICT);
      return;
    }
  }
...
/*    extract_obj(obj); */
  ...
  sprintf(buf, "$n tells you, '&WI'll take &Y%d&W coins for the stamp.&w'\r\n"
       "$n tells you, '&WWrite your message, (&M/s&W saves &C/h&W for help)&w'",
	  price);
  ...
  *(ch->desc->str) = NULL;
  ch->desc->max_str = MAX_MAIL_TEXT_SIZE;
}
			  ...
			  int cmd, char *arg)
{
  ...
  while (has_mail(GET_IDNUM(ch))) {
    ...
    GET_OBJ_COST(obj) = 30;
    GET_OBJ_RENT(obj) = 10;
    obj->action_description = read_delete(GET_IDNUM(ch), &mail_obj);
  ...
  if (STATE(d) == CON_PLAYING)
    if(d->character)
     if (has_mail(GET_IDNUM(d->character))){
      ch = d->character;
      if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_AUTOMAIL) && 
        GET_POS(ch) > POS_SLEEPING && GET_POS(ch) != POS_FIGHTING) {
        while (has_mail(GET_IDNUM(ch))) {
      	    obj = create_obj();
    	    obj->item_number = NOTHING;
    	    ...
    	    if (obj->action_description == NULL)
              obj->action_description = 
	        str_dup("&RMail system error - please report.  Error #11.&w\r\n");

            obj_to_char(obj, ch);
    
            act("&WA fairy pops in and hands you a Postcard.&w",FALSE,ch,0 , 0, TO_CHAR);
...
//              mail_obj = read_object(real_object(obj_vnum), REAL);
              obj_to_char(mail_obj, ch);
              send_to_char("Wow... your mail has an attachment!\ros",ch);
            }
